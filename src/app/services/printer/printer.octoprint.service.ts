...
export class PrinterOctoprintService implements PrinterService {
  ...
  // STELLAMOVE
  public relaseSD(): void {
    const payload: SdCommand = {
      command: 'release',
    };

    this.http
      .post(this.configService.getApiURL('printer/sd'), payload, this.configService.getHTTPHeaders())
      .pipe(
        catchError(error => {
          this.notificationService.setNotification({
            heading: $localize`:@@printer-error-relase:Can't Release SD Card!`,
            text: error.message,
            type: NotificationType.ERROR,
            time: new Date(),
          });
  ...
  public initSD(): void {
    ...
    this.http
      ...
      .pipe(
        catchError(error => {
          this.notificationService.setNotification({
            heading: $localize`:@@printer-error-init:Can't Initialize SD Card!`,
            text: error.message,
            type: NotificationType.ERROR,
            time: new Date(),
          });
  ...
  public refreshSD(): void {
    ...
    this.http
      ...
      .pipe(
        catchError(error => {
          this.notificationService.setNotification({
            heading: $localize`:@@printer-error-refresh:Can't Refresh SD Card!`,
            text: error.message,
            type: NotificationType.ERROR,
            time: new Date(),
          });
