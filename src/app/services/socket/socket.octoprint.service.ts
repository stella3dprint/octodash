...
export class OctoPrintSocketService implements SocketService {
  ...
  //==== Notifications ====//
  // STELLAMOVE
  public changePrinterNotification(notification: PrinterNotification) {
    if (notification.text)
    {
      switch (notification.text)
      {
        case 'Paused':
          this.NotiText = $localize`:@@socket-pause:Paused`;
          break;
        case 'Resuming':
          this.NotiText = $localize`:@@socket-resume:Resuming`;
          break;
        case 'Heater Timeout':
          this.NotiText = $localize`:@@socket-reheat-timeout:Need reheat`;
          break;
        case 'Reheating':
          this.NotiText = $localize`:@@socket-reheat-reheating:Reheating`;
          break;
        case 'Reheat Done':
          this.NotiText = $localize`:@@socket-reheat-done:Reheat Done`;
          break;
        case 'Wait for filament changing':
          this.NotiText = $localize`:@@socket-change:Wait for filament changing`;
          break;
        case 'Insert filament and press button to continue':
          this.NotiText = $localize`:@@socket-insert:Insert filament and press button to continue`;
          break;
        default:
          this.NotiText = notification.text;
          break;
      }
    ...
    if (Object.keys(notification).length > 0) {
      if (notification.action === 'close') {
        ...
        this.notificationService.setNotification({
          ...
          text: this.changePrinterNotification(notification), // notification.text ?? notification.message,
